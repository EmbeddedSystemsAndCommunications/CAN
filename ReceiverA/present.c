...
void present_encrypt(const uint8_t *plain, const uint8_t *key, uint8_t *ans) {
  ...
  round_key[0] = (round_key[0] & 0x0F) | (sbox_present[round_key[0] >> 4] << 4);
  round_key[7] ^= round_counter >> 1;
  round_key[8] ^= round_counter << 7;
  // sBoxLayer
  state[0] = sbox_present[state[0] & 0x0F] | (sbox_present[state[0] >> 4] << 4);
  state[1] = sbox_present[state[1] & 0x0F] | (sbox_present[state[1] >> 4] << 4);
  state[2] = sbox_present[state[2] & 0x0F] | (sbox_present[state[2] >> 4] << 4);
  state[3] = sbox_present[state[3] & 0x0F] | (sbox_present[state[3] >> 4] << 4);
  state[4] = sbox_present[state[4] & 0x0F] | (sbox_present[state[4] >> 4] << 4);
  state[5] = sbox_present[state[5] & 0x0F] | (sbox_present[state[5] >> 4] << 4);
  state[6] = sbox_present[state[6] & 0x0F] | (sbox_present[state[6] >> 4] << 4);
  state[7] = sbox_present[state[7] & 0x0F] | (sbox_present[state[7] >> 4] << 4);
  // pLayer
  buf[0] = state[0];
  buf[1] = state[1];
  buf[2] = state[2];
...
*/
  ...
  for (round_counter = 1; round_counter <= 31; ++round_counter) {
    ...
    round_key[round_counter][9] =
      round_key[round_counter - 1][6] << 5 | round_key[round_counter - 1][7] >> 3;
    round_key[round_counter][8] =
      round_key[round_counter - 1][5] << 5 | round_key[round_counter - 1][6] >> 3;
    round_key[round_counter][7] =
      round_key[round_counter - 1][4] << 5 | round_key[round_counter - 1][5] >> 3;
    round_key[round_counter][6] =
      round_key[round_counter - 1][3] << 5 | round_key[round_counter - 1][4] >> 3;
    round_key[round_counter][5] =
      round_key[round_counter - 1][2] << 5 | round_key[round_counter - 1][3] >> 3;
    round_key[round_counter][4] =
      round_key[round_counter - 1][1] << 5 | round_key[round_counter - 1][2] >> 3;
    round_key[round_counter][3] =
      round_key[round_counter - 1][0] << 5 | round_key[round_counter - 1][1] >> 3;
    round_key[round_counter][2] =
      round_key[round_counter - 1][9] << 5 | round_key[round_counter - 1][0] >> 3;
    round_key[round_counter][1] =
      round_key[round_counter - 1][8] << 5 | round_key[round_counter - 1][9] >> 3;
    round_key[round_counter][0] =
      round_key[round_counter - 1][7] << 5 | round_key[round_counter - 1][8] >> 3;
    round_key[round_counter][0] =
      (round_key[round_counter][0] & 0x0F) | (sbox_present[round_key[round_counter][0] >> 4] << 4);
